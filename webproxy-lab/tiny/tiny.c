...
void read_requesthdrs(rio_t *rp); // 요청 헤더를 빈 줄이 나올 때까지 읽어 버립니다.
...
void doit(int fd)
{
    int is_static; // 정적(1)인지 동적(0)인지 구분하는 플래그입니다.
    struct stat sbuf; // 파일 존재 여부, 크기, 권한 정보를 담습니다.
    char buf[MAXLINE], method[MAXLINE], uri[MAXLINE], version[MAXLINE]; // 요청 라인과 그 안의 토큰들을 저장합니다.
    char filename[MAXLINE], cgiargs[MAXLINE]; // 실제 파일 경로와 CGI 인자를 저장합니다.
    ...
    printf("Request headers:\n"); // 아래부터 들어오는 요청 내용을 로그로 구분해 출력합니다.
    printf("%s", buf); // 방금 읽은 요청 라인을 그대로 출력합니다.
    sscanf(buf, "%s %s %s", method, uri, version); // 요청 라인을 메서드/URI/버전 세 부분으로 나눕니다.

    /* Tiny는 GET 메서드만 처리한다. */
    if (strcasecmp(method, "GET")) { // Tiny는 GET만 지원하므로 다른 메서드는 거절합니다.
        clienterror(fd, method, "501", "Not implemented",
                    "Tiny does not implement this method"); // 지원하지 않는 메서드라는 의미의 501 응답을 보냅니다.
        return; // 이 요청은 더 처리하지 않고 종료합니다.
    }

    read_requesthdrs(&rio); // 요청 헤더 나머지를 모두 읽어 비웁니다.

    /* URI를 파일 이름으로 바꾸고, 정적/동적 콘텐츠 여부를 결정한다. */
    is_static = parse_uri(uri, filename, cgiargs); // URI를 실제 파일 경로와 CGI 인자로 번역합니다.
    if (stat(filename, &sbuf) < 0) { // 해당 파일이 실제로 존재하는지, 정보가 무엇인지 확인합니다.
        clienterror(fd, filename, "404", "Not found",
                    "Tiny couldn't find this file"); // 파일이 없으면 404 응답을 보냅니다.
        return; // 없는 파일은 더 진행할 수 없으므로 종료합니다.
    }

    if (is_static) { // 정적 콘텐츠라면 파일 내용을 그대로 응답합니다.
        /* 일반 파일이면서 읽기 권한이 있어야 정적 파일로 제공할 수 있다. */
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) { // 일반 파일이 아니거나 읽기 권한이 없으면 금지합니다.
            clienterror(fd, filename, "403", "Forbidden",
                        "Tiny couldn't read the file"); // 접근은 됐지만 읽을 수 없는 상황임을 알립니다.
            return; // 권한 문제이므로 응답 후 종료합니다.
        }
        serve_static(fd, filename, sbuf.st_size); // 정적 파일 응답 전체를 전송합니다.
    }
    else { // cgi-bin이 포함된 URI는 동적 콘텐츠 요청으로 처리합니다.
        /* CGI 프로그램은 일반 파일이면서 실행 권한이 있어야 한다. */
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) { // 실행 파일이 아니거나 실행 권한이 없으면 CGI를 돌릴 수 없습니다.
            clienterror(fd, filename, "403", "Forbidden",
                        "Tiny couldn't run the CGI program"); // CGI 실행 불가를 403으로 응답합니다.
            return; // 실행할 수 없으니 종료합니다.
        }
        serve_dynamic(fd, filename, cgiargs); // CGI 프로그램을 실행해 동적 응답을 만듭니다.
    }
}
...
void read_requesthdrs(rio_t *rp)
{
    ...
    while(strcmp(buf, "\r\n")) { // 빈 줄이 나올 때까지 헤더 구간이 계속된다는 뜻입니다.
        ...
        printf("%s", buf); // 학습용으로 실제 헤더 내용을 로그에 출력합니다.
    }
